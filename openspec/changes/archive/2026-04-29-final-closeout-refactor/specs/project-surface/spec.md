...
### Requirement: Configuration directory taxonomy files
**Reason**: Taxonomy files are maintained in the root `taxonomy/` directory; `config/taxonomy/` is redundant.
**Migration**: Use `taxonomy/subjects.yaml` and `taxonomy/resource_types.yaml` directly.

### Requirement: Tests output directories
**Reason**: `tests/output-round-1/` and `tests/output-round-2/` contain test artifacts that should be generated at runtime and ignored.
**Migration**: Test outputs are now ignored via `.gitignore` and generated on-demand.

### Requirement: Opencode configuration directory
**Reason**: `.omc/` directory contains Opencode-specific configuration that should not be part of this project.
**Migration**: No migration needed; this was external tooling configuration.
