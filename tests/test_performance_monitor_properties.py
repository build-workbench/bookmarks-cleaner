...
def create_monitor(latency_threshold_ms=100, window_size=10000):
    ...
    return PerformanceMonitor(
        {"latency_threshold_ms": latency_threshold_ms, "window_size": window_size}
    )
...
class TestLatencyPercentileAccuracy:
    ...
    def test_latency_percentile_accuracy(self, latencies):
        ...
        for latency in latencies:
            monitor.record_classification(
                method="test_method", latency_ms=latency, confidence=0.8
            )
        ...
        # 验证百分位数（允许小误差）
        assert (
            abs(percentiles["p50"] - expected_p50) < 0.01
        ), f"p50 should be {expected_p50}, got {percentiles['p50']}"
        assert (
            abs(percentiles["p95"] - expected_p95) < 0.01
        ), f"p95 should be {expected_p95}, got {percentiles['p95']}"
        assert (
            abs(percentiles["p99"] - expected_p99) < 0.01
    ...
    def test_percentile_ordering(self, latencies):
        ...
        for latency in latencies:
            monitor.record_classification(
                method="test", latency_ms=latency, confidence=0.8
            )
...
class TestLatencyAlertEmission:
    ...
    def test_alert_callback(self):
        ...
        # 触发告警
        monitor.record_classification(
            method="test", latency_ms=100, confidence=0.8  # 超过阈值
        )
...
class TestPrometheusFormatValidity:
    ...
    def test_prometheus_format_validity(self, methods, n_records):
        ...
        for method in methods:
            for _ in range(n_records):
                monitor.record_classification(
                    method=method, latency_ms=50.0, confidence=0.8
                )
        ...
        for line in lines:
            ...
            # 验证行格式
            assert self.PROMETHEUS_LINE_PATTERN.match(
                line
    ...
    def test_prometheus_contains_required_metrics(self):
        ...
        # 记录一些数据
        monitor.record_classification(
            method="test_method", latency_ms=50.0, confidence=0.8
        )
        monitor.record_cache_access(hit=True)
...
class TestMethodAccuracyTracking:
    ...
    def test_method_accuracy_calculation(self, correct_count, incorrect_count):
        ...
        for _ in range(correct_count):
            monitor.record_classification(
                method="test_method", latency_ms=50.0, confidence=0.8, correct=True
            )

        # 记录错误的分类
        for _ in range(incorrect_count):
            monitor.record_classification(
                method="test_method", latency_ms=50.0, confidence=0.8, correct=False
            )
...
class TestConfidenceDistribution:
    ...
    def test_confidence_distribution(self, confidences):
        ...
        for conf in confidences:
            monitor.record_classification(
                method="test_method", latency_ms=50.0, confidence=conf
            )
        ...
        assert total_in_distribution == len(
            confidences
