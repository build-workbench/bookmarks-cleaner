...
class MockClassifierPlugin(ClassifierPlugin):
    ...
    def classify(self, features: BookmarkFeatures) -> ClassificationResult:
        return ClassificationResult(
            category=self._category, confidence=self._confidence, method=self._name
        )
...
def create_pipeline(registry=None, fusion_strategy="weighted_voting"):
    ...
    return ClassifierPipeline(registry, {"fusion_strategy": fusion_strategy})
...
class TestFusionStrategyApplication:
    ...
    def test_fusion_strategy_application(self, strategy, categories, confidences):
        ...
        pipeline = ClassifierPipeline(registry, {"fusion_strategy": strategy})

        # 注册并启用插件
        for i, (cat, conf) in enumerate(zip(categories, confidences)):
            plugin = MockClassifierPlugin(
                name=f"plugin_{i}", category=cat, confidence=conf, priority=i * 10
            )
        ...
        # 验证使用了正确的策略
        assert pipeline.fusion_strategy == FusionStrategy(
            strategy
    ...
    def test_weighted_voting_selects_highest_score(self):
        ...
        """
        registry = create_registry()
        pipeline = ClassifierPipeline(registry, {"fusion_strategy": "weighted_voting"})

        # 创建两个插件，一个高置信度，一个低置信度
        high_conf_plugin = MockClassifierPlugin(
            name="high_conf", category="高置信度类别", confidence=0.9
        )
        low_conf_plugin = MockClassifierPlugin(
            name="low_conf", category="低置信度类别", confidence=0.3
        )
...
class TestDynamicWeightAdjustment:
    ...
    def test_multiple_methods_independent_weights(self):
        ...
        # 高准确率方法应该有更高权重
        assert weight_a > weight_b, "Higher accuracy method should have higher weight"
    ...
    def test_conflict_resolution_with_rules(self):
        ...
        pipeline = ClassifierPipeline(
            registry,
            {
                ...
                "conflict_rules": {"prefer_rule_engine": "prefer_rule_engine"},
            },
        )

        # 创建规则引擎插件和其他插件
        rule_plugin = MockClassifierPlugin(
            name="rule_classifier", category="规则类别", confidence=0.6
        )
        other_plugin = MockClassifierPlugin(
            name="other_classifier", category="其他类别", confidence=0.7
        )
...
class TestFusionWithSinglePlugin:
    ...
    @settings(max_examples=30)
    @given(category=category_strategy, confidence=confidence_strategy)
    def test_single_plugin_passthrough(self, category, confidence):
        ...
        plugin = MockClassifierPlugin(
            name="single_plugin", category=category, confidence=confidence
        )
