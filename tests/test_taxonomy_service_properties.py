...
class TestTaxonomyYAMLRoundTrip:
    ...
    def test_taxonomy_yaml_round_trip(self, categories):
        ...
        try:
            categories = [
                c for c in categories if len(c.strip()) > 0 and service.validate_name(c)
            ]
...
class TestCategoryNameValidation:
    ...
    def test_invalid_category_names_rejected(self, name):
        ...
        try:
            ...
            assert (
                result is False
            ), f"Should not be able to add invalid category '{name}'"
        finally:
            shutil.rmtree(temp_dir, ignore_errors=True)
...
class TestCategoryRenamePropagation:
    ...
    @settings(max_examples=50)
    @given(old_name=valid_name_strategy, new_name=valid_name_strategy)
    def test_category_rename_propagation(self, old_name, new_name):
        ...
        try:
            ...
            assert (
                result is True
            ), f"Rename from '{old_name}' to '{new_name}' should succeed"
...
class TestCategoryMergeCompleteness:
    ...
    @settings(max_examples=50)
    @given(source=valid_name_strategy, target=valid_name_strategy)
    def test_category_merge_completeness(self, source, target):
        ...
        try:
            ...
            assert (
                source in variants
            ), f"Source '{source}' should be a variant of target"
