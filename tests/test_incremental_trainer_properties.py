...
@pytest.mark.skipif(
    not IMPORTS_AVAILABLE or not JOBLIB_AVAILABLE,
...
class TestIncrementalUpdateTrigger:
    ...
    def test_incremental_update_trigger(self, batch_size, n_samples):
        ...
        try:
            trainer = IncrementalTrainer(
                {"batch_size": batch_size, "model_dir": temp_dir, "max_versions": 5}
            )
            trainer.set_model(MockModel())
            ...
            for i in range(n_samples):
                trainer.add_sample(
                    features={"text": f"sample {i}", "value": 0.5}, label="测试分类"
                )
            ...
            assert (
                trainer.get_pending_count() == expected_pending
    ...
    def test_force_update(self):
        ...
        try:
            # 添加少于 batch_size 的样本
            for i in range(3):
                trainer.add_sample(features={"text": f"sample {i}"}, label="测试")
...
class TestModelVersionHistory:
    ...
    def test_model_version_history(self, n_updates):
        ...
        try:
            trainer = IncrementalTrainer(
                {"batch_size": 2, "model_dir": temp_dir, "max_versions": max_versions}
            )
            trainer.set_model(MockModel())
...
class TestVersionCleanup:
    ...
    def test_old_versions_cleaned_up(self, max_versions, n_updates):
        ...
        try:
            trainer = IncrementalTrainer(
                {"batch_size": 1, "model_dir": temp_dir, "max_versions": max_versions}
            )
            trainer.set_model(MockModel())
