...
class FailingPlugin(ClassifierPlugin):
    def __init__(self, name: str, should_fail: bool = False):
        self._metadata = PluginMetadata(
            name=name, version="1.0", capabilities=["classification"], priority=100
        )
    ...
    def classify(self, features) -> Optional[MockClassificationResult]:
        ...
        return MockClassificationResult(
            category=f"category_{self._metadata.name}", confidence=0.8
        )
...
class WorkingPlugin(ClassifierPlugin):
    ...
    def classify(self, features) -> Optional[MockClassificationResult]:
        if not self._initialized:
            return None
        return MockClassificationResult(
            category=self._category, confidence=self._confidence
        )
...
def test_pipeline_weighted_voting():
    ...
    pipeline = ClassifierPipeline(
        registry,
        {
            ...
            "method_weights": {"plugin1": 1.0, "plugin2": 1.0, "plugin3": 1.0},
        },
    )
