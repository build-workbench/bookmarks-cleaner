...
@given(key=key_strategy, embedding=embedding_strategy)
def test_property_17_ttl_expiration(key, embedding):
    ...
    store = FeatureStore({"max_size": 1000, "ttl_seconds": ttl_seconds})
...
@given(
    items=st.lists(
        st.tuples(key_strategy, embedding_strategy), min_size=5, max_size=20
    ),
...
def test_property_18_lru_eviction_policy(items, max_size):
    ...
    store = FeatureStore({"max_size": max_size, "ttl_seconds": 3600})
...
def test_feature_store_persist_and_load(tmp_path):
    ...
    store1 = FeatureStore(
        {"max_size": 100, "ttl_seconds": 3600, "persist_path": persist_path}
    )
