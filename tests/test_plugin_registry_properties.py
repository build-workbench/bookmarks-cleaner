...
@given(name=plugin_name_strategy, version=version_strategy, priority=priority_strategy)
...
@given(
    priorities=st.lists(
        st.integers(min_value=0, max_value=100), min_size=2, max_size=10, unique=False
    )
)
def test_property_2_plugin_invocation_order(priorities):
    ...
    assert plugin_priorities == sorted(
        plugin_priorities
...
@settings(max_examples=100)
@given(name=plugin_name_strategy, version=version_strategy)
