...
def create_bookmark(
    url: str = "https://example.com/page", title: str = "Example"
) -> dict:
    """创建书签字典"""
    return {"id": str(hash(url)), "url": url, "title": title}
...
class TestLowConfidenceDetection:
    ...
    def test_high_confidence_not_queued(self, confidence, category):
        ...
        result = engine.process_classification(
            bookmark=bookmark, category=category, confidence=confidence, alternatives=[]
        )
...
class TestUncertaintySampling:
    ...
    def test_uncertainty_sampling_priority(self, confidences):
        ...
        for i, conf in enumerate(confidences):
            bookmark = create_bookmark(
                url=f"https://example.com/page{i}", title=f"Title {i}"
            )
            # 创建不同的备选分类以产生不同的不确定性
        ...
        for i in range(len(items) - 1):
            assert (
                items[i].uncertainty_score >= items[i + 1].uncertainty_score
...
class TestSessionRequestLimit:
    ...
    def test_session_request_limit(self, max_requests):
        ...
        for i in range(max_requests + 10):
            ...
            engine.process_classification(
                bookmark=bookmark, category="测试", confidence=0.3, alternatives=[]
            )
...
class TestFeedbackPersistence:
    ...
    @settings(max_examples=100)
    @given(
        bookmark_id=st.text(min_size=1, max_size=50), correct_category=category_strategy
    )
    def test_feedback_persistence(self, bookmark_id, correct_category):
        ...
        assert len(samples) == initial_count + 1, "Labeled sample should be added"
    ...
    @given(
        feedbacks=st.lists(
            st.tuples(st.text(min_size=1, max_size=20), category_strategy),
            min_size=1,
            max_size=10,
        )
    )
    def test_multiple_feedback_persistence(self, feedbacks):
        ...
        for bookmark_id, category in feedbacks:
            engine.submit_feedback(bookmark_id=bookmark_id, correct_category=category)
...
class TestUncertaintyCalculation:
    ...
    def test_high_confidence_low_uncertainty(self):
        ...
        assert (
            high_conf_uncertainty <= low_conf_uncertainty
