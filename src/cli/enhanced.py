...
class EnhancedCLI:
    ...
    def print_header(self, title: str, subtitle: str = None):
        ...
        if self.use_rich:
            ...
            panel = Panel(text, border_style="magenta", padding=(1, 2))
    ...
    def print_table(
        self, data: List[Dict], title: str = None, headers: List[str] = None
    ):
        """打印表格"""
        if not data:
    ...
    def prompt(
        self, question: str, default: str = None, choices: List[str] = None
    ) -> str:
        """输入提示"""
        if self.use_rich:
...
class InteractiveBookmarkManager:
    ...
    def run(self):
        ...
        self.cli.print_header(
            "🚀 智能书签分类系统", "Enhanced Bookmark Classification System v2.0"
        )
    ...
    def view_results(self):
        ...
        if os.path.exists(output_dir):
            ...
            if files:
                selected_file = self.cli.select_from_list(files, "选择要查看的结果文件")
                ...
                self.cli.print_stats(
                    {
                        ...
                        "AI设置": json.dumps(
                            config.get("ai_settings", {}), ensure_ascii=False
                        ),
                    },
                    "当前配置",
                )
...
更多信息请访问项目文档。
        ...
        if self.cli.use_rich:
            self.cli.console.print(
                Panel(help_text.strip(), title="帮助信息", border_style="green")
            )
        else:
            print(help_text)
