...
class URLAnalyzer:
    ...
    def _identify_site_type(
        self, domain: str, subdomain: str, path: str, path_segments: List[str]
    ) -> Tuple[str, str]:
        """识别站点类型"""
    ...
    def _identify_video_content(
        self, domain: str, path: str, path_segments: List[str]
    ) -> str:
        """识别视频内容类型"""
    ...
    def _detect_language(
        self, analysis: URLAnalysis, path: str, path_segments: List[str]
    ):
        """检测编程语言"""
        # 从文件扩展名检测
        if path_segments:
