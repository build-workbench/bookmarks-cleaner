...
# 导入机器学习分类器
try:
    from src.classifiers.ml import ML_AVAILABLE, MLClassifierWrapper
...
class EnhancedClassifier:
    ...
    def _detect_content_type(
        self, url: str, title: str, path_segments: List[str]
    ) -> str:
        """智能内容类型检测"""
        url_lower = url.lower()
        ...
        if self.compiled_patterns.get("github_repo", re.compile(r"")).search(url_lower):
            if any(
                keyword in path_segments for keyword in ["issues", "pull", "releases"]
            ):
        ...
        if self.compiled_patterns.get("api_endpoint", re.compile(r"")).search(
            url_lower
        ):
            return "api_docs"
        ...
        for content_type, keywords in type_indicators.items():
            if any(
                keyword in title_lower or keyword in url_lower for keyword in keywords
            ):
                return content_type
    ...
    def classify(
        ...
        try:
            ...
            if not scores:
                ...
                # 应用置信度提升因子
                boost_factor = self.config.get("advanced_settings", {}).get(
    ...
    def _apply_priority_rules(
        self, features: EnhancedBookmarkFeatures
    ) -> Dict[str, float]:
        """应用优先级规则"""
        scores = defaultdict(float)
    ...
    def _apply_context_rules(
        self, features: EnhancedBookmarkFeatures, context: Optional[Dict]
    ) -> Dict[str, float]:
        """应用上下文感知规则"""
        scores = defaultdict(float)
        ...
        if 9 <= current_hour <= 17:  # 工作时间
            if any(
                work_indicator in features.domain
                for work_indicator in ["github.com", "stackoverflow.com"]
            ):
                scores["技术栈"] += 2.0
    ...
    def _apply_temporal_patterns(
        ...
        for category, timestamps in self.temporal_patterns.items():
            recent_count = sum(
                1 for ts in timestamps if current_time - ts < timedelta(hours=24)
            )
    ...
    def _evaluate_enhanced_rule(
        self, rule: Dict, features: EnhancedBookmarkFeatures, base_weight: float
    ) -> float:
        """增强的规则评估"""
    ...
    def _calculate_match_score(
        self, keyword: str, target_text: str, features: EnhancedBookmarkFeatures
    ) -> float:
        """计算匹配分数"""
    ...
    def learn_from_feedback(
        ...
        # 更新学习权重
        learning_rate = self.config.get("advanced_settings", {}).get(
            "learning_rate", 0.1
        )
        ...
        if len(self.temporal_patterns[correct_category]) > 100:
            self.temporal_patterns[correct_category] = self.temporal_patterns[
                correct_category
            ][-100:]
    ...
    def save_learning_data(self, path: str = "models/learning_data.json"):
        ...
        learning_data = {
            ...
            "category_patterns": {
                k: list(v) for k, v in self.category_patterns.items()
            },
            "temporal_patterns": {
    ...
    def load_learning_data(self, path: str = "models/learning_data.json"):
        ...
        try:
            ...
            self.learning_weights = defaultdict(
                float, learning_data.get("learning_weights", {})
            )
