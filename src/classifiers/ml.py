    ...
    def to_array(self) -> np.ndarray:
        ...
        numerical_features = np.array(
            [
                ...
                self.has_numbers,
                self.has_chinese,
                self.content_type_encoded,
                self.language_encoded,
            ]
        )
...
class BookmarkFeatureExtractor(BaseEstimator, TransformerMixin):
    ...
    def fit(self, bookmarks, y=None):
        ...
        content_types = [
            bookmark.get("content_type", "unknown") for bookmark in bookmarks
        ]
...
class MLBookmarkClassifier:
    ...
    def _create_models(self):
        ...
        # 6. SGD (用于在线学习)
        models["sgd"] = SGDClassifier(loss="log_loss", random_state=42)
    ...
    def train(self, validation_split=0.2, optimize_hyperparams=False):
        ...
        try:
            ...
            if small_categories:
                ...
                # 用一个统一的分类来替换小分类
                processed_labels = [
                    "_MERGED_CATEGORY_" if label in small_categories else label
            ...
            if self.use_ensemble and "ensemble" in self.models:
                ...
                # 选择准确率最高的模型
                best_model_name = max(
                    self.training_stats["accuracy_scores"].items(), key=lambda x: x[1]
                )[0]
            ...
            self.training_stats.update(
                {
                    ...
                    "sklearn_version": getattr(
                        sys.modules.get("sklearn"), "__version__", None
                    ),
                }
            )
    ...
    def evaluate_model(
        self, test_bookmarks: List[Dict], test_labels: List[str]
    ) -> Dict:
        """评估模型性能"""
        if not ML_AVAILABLE or self.ensemble_model is None:
            return {}
        ...
        try:
            # 预测
            predictions = [
                self.predict_single(bookmark)[0] for bookmark in test_bookmarks
            ]
    ...
    def save_model(self):
        ...
        try:
            ...
            # 保存标签编码器
            joblib.dump(
                self.label_encoder, os.path.join(self.model_dir, "label_encoder.pkl")
            )

            # 保存统计信息
            with open(
                os.path.join(self.model_dir, "training_stats.json"),
                "w",
                encoding="utf-8",
            ) as f:
...
class MLClassifierWrapper:
    ...
    def __init__(self, model_dir="models/ml"):
        if not ML_AVAILABLE:
            raise ImportError(
                "机器学习依赖不可用，请运行: pip install scikit-learn jieba langdetect"
            )
