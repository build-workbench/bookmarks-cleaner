...
# 导入 URL 分析器
try:
    from src.engines.url import URLAnalysis, URLAnalyzer
except ImportError:
    URLAnalyzer = None
    URLAnalysis = None
...
class RuleEngine:
        ...
        for section in processing_order:
            ...
            for category, category_data in section_rules.items():
                ...
                for i, rule in enumerate(rules):
                    ...
                    match_all_keywords_in = (rule or {}).get(
                            ...
                            for kw in field_keywords:
                                try:
                                    ...
                                    field_patterns.append(
                                        re.compile(escaped_kw, re.IGNORECASE)
                                    )
                                except re.error:
                                    continue
                            if field_patterns:
        ...
        try:
            ...
            try:
                ...
                if any(
                    ...
                    p in url_lower for p in ["docs.", "/docs", "documentation", "wiki"]
                ):
                    resource_type_hint = resource_type_hint or "documentation"
                elif any(
                    d in domain for d in ["youtube.com", "bilibili.com", "vimeo.com"]
                ):
            ...
            facets = (
                {"resource_type_hint": resource_type_hint} if resource_type_hint else {}
            )
    ...
    def _generate_reasoning(
        self, matches: List[RuleMatch], best_category: str
    ) -> List[str]:
        """生成推理过程"""
        reasoning = []
    ...
    def add_dynamic_rule(
        self, category: str, match_type: str, keyword: str, weight: float = 1.0
    ):
        """动态添加规则"""
        if category not in self.compiled_rules:
            self.compiled_rules[category] = []
        ...
        for category, rules in self.compiled_rules.items():
            for rule in rules:
                if rule["rule_id"] == rule_id:
                    ...
                    self.logger.info(
                        f"更新规则权重: {rule_id} {old_weight} -> {new_weight}"
                    )
                    return True
    ...
    def get_rule_performance(self) -> Dict:
        ...
        return {
            ...
            "top_rules": dict(
                sorted(
                    self.stats["rule_hits"].items(), key=lambda x: x[1], reverse=True
                )[:10]
            ),
