...
from .category import normalize_category_config
...
class ConfigValidator:
    ...
    def validate(self, config: Dict) -> List[str]:
        ...
        try:
            ...
            errors.append(
                f"配置格式错误: {e.message} at {'.'.join(str(p) for p in e.absolute_path)}"
            )
...
class EnhancedConfigManager:
    ...
    def _apply_env_overrides(self, config: Dict) -> Dict:
        ...
        # 环境变量映射
        env_mappings = {
            f"{self.env_prefix}CLASSIFICATION_THRESHOLD": (
                "ai_settings",
                "confidence_threshold",
                float,
            ),
            f"{self.env_prefix}LEARNING_RATE": (
                "advanced_settings",
                "learning_rate",
                float,
            ),
            f"{self.env_prefix}MAX_CATEGORIES": (
                "advanced_settings",
                "max_categories",
                int,
            ),
            f"{self.env_prefix}CACHE_SIZE": ("ai_settings", "cache_size", int),
            f"{self.env_prefix}SHOW_CONFIDENCE": (
                None,
        ...
        for key, new_value in new_config.items():
            ...
            if key not in old_config:
                ...
                if isinstance(new_value, dict) and isinstance(old_config[key], dict):
                    # 递归检查嵌套字典
                    changes.extend(
                        self._find_changes(old_config[key], new_value, current_path)
                    )
                else:
                    # 修改
                    changes.append(
    ...
    def get_changes_since(self, timestamp: datetime) -> List[ConfigChange]:
        """获取指定时间以来的变更"""
        return [
            change for change in self.config_history if change.timestamp > timestamp
        ]
    ...
    def get_stats(self) -> Dict:
        ...
        return {
            ...
            "last_reload": (
                max(change.timestamp for change in self.config_history)
...
def get_config(path: Optional[str] = None, default=None):
    """获取配置值的便捷函数"""
