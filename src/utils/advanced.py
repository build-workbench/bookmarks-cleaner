...
class IntelligentDeduplicator:
    ...
    def _calculate_similarity(
        self, bookmark1: Dict, bookmark2: Dict
    ) -> SimilarityScore:
        """计算两个书签的相似度"""
    ...
    def _normalize_url(self, url: str) -> str:
        ...
        try:
            ...
            # 重构URL
            normalized = (
                f"{parsed.scheme}://{parsed.netloc.replace('www.', '')}{parsed.path}"
            )
            if query:
                normalized += f"?{query}"
    ...
    def suggest_best_bookmark(self, duplicate_group: List[Dict]) -> Dict:
        ...
        def score_bookmark(bookmark):
            ...
            if any(
                trusted in domain
                for trusted in ["github.com", "stackoverflow.com", "wikipedia.org"]
            ):
                score += 2
    ...
    def remove_duplicates(
        self, bookmarks: List[Dict], interactive=False
    ...
    def _interactive_selection(self, group: List[Dict]) -> Dict:
        ...
        while True:
            choice = input(
                f"请选择保留哪个 (1-{len(group)}, 回车选择推荐的 #{suggested_index}): "
            ).strip()
...
class PersonalizedRecommendationSystem:
    ...
    def recommend_categories(
        self, url: str, title: str, n_recommendations: int = 3
        ...
        for category, hours in self.time_patterns.items():
            if hours:
                ...
                scores[category] += (
                    time_similarity * self.category_preferences[category] * 0.2
                )
    ...
    def recommend_similar_bookmarks(
        self,
        target_bookmark: Dict,
    ...
    def get_trending_categories(self, days: int = 7) -> List[Tuple[str, int]]:
        ...
        # 这里简化处理，实际应该基于时间序列数据
        return sorted(
            self.category_preferences.items(), key=lambda x: x[1], reverse=True
        )[:10]
    ...
    def _load_model(self):
        ...
        if os.path.exists(self.model_path):
            try:
                ...
                self.category_preferences = defaultdict(
                    float, model_data.get("category_preferences", {})
                )
                self.domain_preferences = defaultdict(
                    float, model_data.get("domain_preferences", {})
                )
                self.time_patterns = defaultdict(
                    list, model_data.get("time_patterns", {})
                )
...
class BookmarkHealthChecker:
    ...
    def check_bookmarks(
        self, bookmarks: List[Dict], progress_callback=None
    ) -> List[BookmarkHealth]:
        """批量检查书签健康状态"""
    ...
    def get_health_summary(self, health_results: List[BookmarkHealth]) -> Dict:
        ...
        avg_response_time = sum(
            h.response_time for h in health_results if h.is_accessible
        )
