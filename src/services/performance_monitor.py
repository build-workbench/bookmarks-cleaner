...
class PerformanceMonitor:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        # 方法统计
        self._method_stats: Dict[str, Dict] = defaultdict(
            lambda: {
                "calls": 0,
                "correct": 0,
    ...
    def record_memory_usage(self, bytes_used: int):
        ...
        with self._lock:
            self._memory_samples.append(
                {"timestamp": datetime.now(), "bytes": bytes_used}
            )
    ...
    def _emit_alert(
        self, alert_type: str, message: str, details: Optional[Dict] = None
    ):
        """
        发出告警
    ...
    def export_prometheus_metrics(self) -> str:
        ...
        with self._lock:
            for method, stats in self._method_stats.items():
                ...
                lines.append(
                    f'classification_accuracy{{method="{safe_method}"}} {accuracy}'
                )
                lines.append(
                    f'classification_calls{{method="{safe_method}"}} {stats["calls"]}'
                )
        ...
        with self._lock:
            ...
            return {
                ...
                "total_classifications": sum(
                    s["calls"] for s in self._method_stats.values()
                ),
            }
