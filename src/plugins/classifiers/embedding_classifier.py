...
class EmbeddingClassifier(ClassifierPlugin):
    ...
    def initialize(self, config: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"EmbeddingClassifier initialized with {len(self._category_prototypes)} categories"
            )
            return True
    ...
    def classify(self, features: BookmarkFeatures) -> Optional[ClassificationResult]:
        ...
        try:
            ...
            # 生成备选类别
            alternatives = [
                (cat, sim) for cat, sim in similarities.items() if cat != best_category
            ]
            ...
            return ClassificationResult(
                ...
                reasoning=[
                    f"Cosine similarity with '{best_category}' prototype: {best_similarity:.3f}"
                ],
    ...
    def add_category_prototype(self, category: str, texts: List[str]):
        ...
        self._category_prototypes[category] = prototype
        self.logger.info(
            f"Added prototype for category '{category}' from {len(texts)} texts"
        )

    def update_prototype(
        self, category: str, new_embedding: np.ndarray, alpha: float = 0.1
    ):
        """
        增量更新类别原型
        ...
        if category in self._category_prototypes:
            ...
            self._category_prototypes[category] = (
                1 - alpha
            ) * old_prototype + alpha * new_embedding
        else:
