const landingContent = {
  en: {
    eyebrow: 'System guide for an offline-first bookmark classification CLI',
    abstract: 'Bookmarks Cleaner is documented as a systems artifact: architecture boundaries, classifier cooperation, performance trade-offs, and the reasoning behind a rules-first fusion stack.',
    theses: [
      'Rules-first classification keeps common cases deterministic and inspectable.',
      'Fusion integrates ML, semantics, and optional LLM assistance without making them mandatory.',
      'The site is organized for readers who want a runtime model before implementation detail.',
    ],
    metrics: [
      { label: 'Throughput', value: '420-650/s', note: 'Bookmarks per second on Ryzen 5 5600X' },
      { label: 'Cold start', value: '<100 ms', note: 'Deferred heavy dependencies' },
      { label: 'Runtime modes', value: 'Rules · ML · LLM', note: 'Optional intelligence stack, same CLI surface' },
      { label: 'Outputs', value: 'HTML · JSON · MD', note: 'Human report plus machine-readable exports' },
    ],
    evidence: [
      { title: 'Fusion architecture', detail: 'Weighted voting keeps heterogeneous classifiers interpretable and avoids a second training layer.', href: '/en/algorithms/fusion' },
      { title: 'Offline guarantee', detail: 'The primary execution path remains local. Optional LLM integration is additive, not foundational.', href: '/en/whitepaper' },
      { title: 'Composable runtime', detail: 'Facade, coordinator, pipelines, and protocol boundaries reduce change blast radius.', href: '/en/architecture/pipeline' },
      { title: 'Benchmark framing', detail: 'Performance pages explain the assumptions behind throughput, concurrency, and cache behavior.', href: '/en/performance/optimization' },
    ],
    paths: [
      { index: '01', title: 'Whitepaper first', detail: 'Read the thesis, system boundary, and failure model before diving into code-level mechanics.', href: '/en/whitepaper' },
    ...
    citations: [
      ...
      { title: 'Zadrozny & Elkan 2001', detail: 'Confidence calibration grounding for probability shaping.', href: '/en/resources/references' },
      { title: 'Related projects', detail: 'Compare deployment and intelligence models against linkding and Shaarli.', href: '/en/resources/related-projects' },
      { title: 'Evolution log', detail: 'Trace the move from god class to facade-plus-pipeline boundaries.', href: '/en/evolution' },
    ],
  },
  zh: {
...
export function getLandingContent(lang) {
  return landingContent[lang] ?? landingContent.en
}
