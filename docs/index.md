...
hero:
  name: CleanBook
  text: 开发者的智能书签工具
  textAlt: Smart Bookmark Tool for Developers
  tagline: 规则优先 · ML 辅助 · LLM 可选 · 离线可用<br>将混乱的浏览器书签整理成结构化的知识库
  taglineAlt: Rules-first · ML-assisted · LLM-optional · Offline-ready<br>Transform chaotic browser bookmarks into structured knowledge
  image:
    src: /logo.svg
    alt: CleanBook Logo
  actions:
...
features:
  - icon: 🚀
    title: 默认离线运行
    titleAlt: Offline by Default
    details: 不依赖任何云服务，所有处理在本地完成。你的书签数据永远不会离开你的设备，适合隐私敏感场景。
    detailsAlt: No cloud services required. All processing happens locally. Your bookmark data never leaves your device.
  - icon: ⚙️
    title: 配置驱动
    titleAlt: Config-Driven
    details: 通过 config.json 和 YAML 词表自定义分类规则、阈值和输出格式，无需修改代码即可适配你的工作流。
    detailsAlt: Customize rules, thresholds, and output formats via config.json and YAML taxonomies without code changes.
  - icon: 🤖
    title: 智能分类
    titleAlt: Smart Classification
    details: 融合规则引擎 + ML + 语义分析的多层分类策略，准确率达 91.4%，自动降级保证可用性。
    detailsAlt: Multi-layer fusion of Rule Engine + ML + Semantic Analysis achieves 91.4% accuracy with automatic fallback.
  - icon: 📦
    title: 多格式导出
    titleAlt: Multi-Format Export
    details: 支持 HTML（浏览器导入）、JSON（数据分析）、Markdown（知识库）等多种格式，满足不同场景需求。
    detailsAlt: Support HTML (browser import), JSON (data analysis), Markdown (knowledge base), and more formats.
  - icon: 💻
    title: CLI 优先
    titleAlt: CLI-First
    details: 提供 cleanbook 命令行工具和 cleanbook-wizard 交互向导，支持批处理和自动化集成。
    detailsAlt: Provides cleanbook CLI and cleanbook-wizard interactive mode for batch processing and automation.
  - icon: 🔧
    title: 开源免费
    titleAlt: Open Source
    details: MIT 许可，完全免费使用。支持自定义分类法、规则和导出模板，可扩展的插件架构。
    detailsAlt: MIT licensed and completely free. Extensible plugin architecture with customizable taxonomies.
---

<script setup>
import { ref, onMounted, onUnmounted } from 'vue'

// Check browser language for redirect
onMounted(() => {
  const userLang = navigator.language || navigator.userLanguage
  if (userLang.startsWith('zh')) {
    // Stay on Chinese page (root = zh)
  } else {
    // Redirect to English version
    window.location.href = '/bookmarks-cleaner/en/'
  }
})
...
- [快速开始](/zh/quickstart) — 10 分钟上手
- [安装指南](/zh/guide/installation) — 详细安装说明
- [配置详解](/zh/reference/config) — 自定义你的分类规则
- [设计概述](/zh/design/overview) — 理解系统架构
