...
本次我们共同完成的 `CleanBook` 项目，旨在通过自动化的方式解决书签"熵增"的问题。这份文档将在此基础上，提供一套从日常习惯到工具使用，再到高级技巧的最佳实践，帮助您建立一个高效、有序且真正服务于您的个人知识库。
...
2.  **放入工作目录：** 将导出的 HTML 文件放在任意便于管理的位置，或直接使用项目内 `examples/` 中的示例文件熟悉流程。
3.  **一键执行：** 运行当前主入口：
    ```bash
    python main.py -i chrome_bookmarks.html -o output
    ```
4.  **分析"新生事物"：** 运行后，重点关注 `output/` 下生成的 JSON / Markdown 报告，看看哪些网站是最近频繁访问但尚未被稳定分类的。
5.  **调优"配置文件" (可选)：** 打开 `config.json`，将上一步发现的新模式，以域名或关键词的形式，添加到对应的分类规则中。这是让系统"进化"的关键。
6.  **导入"成品"：** 将 `output/` 中整理好的 HTML 结果导入到一个干净的浏览器文件夹（或一个全新的浏览器 Profile）中，享受清爽的成果。
