...
当前项目的主路径是 `main.py -> src/bookmark_processor.py -> src/ai_classifier.py`，采用基于 `config.json` 的规则引擎，并可选叠加本地 ML 与 LLM。该引擎通过关键词匹配、域名判断和加权评分等方式对书签进行分类。
...
graph TD
    ...
    subgraph "阶段三：集成与应用"
        I[main.py / src.ai_classifier] --> J{6. 加载模型<br/>bookmark_classifier.joblib};
        J --> K[7. 改造分类函数];
        subgraph "混合分类策略"
            direction TB
...
3.  **集成与应用**：修改主路径（`main.py` / `src/ai_classifier.py`），使其能够加载并使用训练好的模型。我们将实现一个混合分类逻辑：优先应用一些简单的、确定性高的规则，对于其他书签，则调用 AI 模型来预测其最佳分类。
...
2.  **数据获取与标注流程（冷启动）**：
    a. **初步运行**：运行 `python main.py -i examples/demo_bookmarks.html -o output`，让现有主路径先输出一轮结果。
    b. **获取初步结果**：脚本会在 `output/` 下生成 HTML / JSON / Markdown 结果。
    c. **人工校对**：
...
1.  **加载模型**：在当前主路径（`main.py` / `src/ai_classifier.py`）启动阶段，检查 `bookmark_classifier.joblib` 文件是否存在，如果存在则使用 `joblib.load` 加载。
